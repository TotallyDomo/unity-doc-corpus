...
func transformOne(sourceAbs string, titles map[string]map[string]string, job transformJob, timer *stageTimer) transformResult {
	var raw []byte
...
func build(source, output string, workers int, keepSource bool) error {
	...
	pageTextStmt, err := tx.Prepare("INSERT INTO page_text(page_key, md) VALUES (?, ?)")
	if err != nil {
		return err
	}
	...
	for i := 0; i < workers; i++ {
		...
		go func() {
			defer wg.Done()
			for job := range jobCh {
				results[job.Index] = transformOne(sourceAbs, titles, job, timer)
			}
		}()
	}
...
2. Search `+"`docs.sqlite`"+` FTS for concept queries.
3. Read the page Markdown with `+"`unity-doc-corpus page <page_key>`"+`.
